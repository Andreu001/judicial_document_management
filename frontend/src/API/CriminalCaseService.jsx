...
class CriminalCaseService {
  ...
  static async update(businesscardId, criminalData) {
    try {
      const response = await baseService.patch(`${BASE_URL}${businesscardId}/criminal/`, criminalData);
      return response.data;
    } catch (error) {
      console.error('Error updating criminal case:', error);
      throw error;
  ...
  static async getDecisions(businesscardId) {
    try {
      const response = await baseService.get(`${BASE_URL}${businesscardId}/decisions/`);
      return response.data;
    } catch (error) {
      if (error.response?.status === 404) {
        return [];
  ...
  static async createDecision(businesscardId, decisionData) {
    try {
      const response = await baseService.post(`${BASE_URL}${businesscardId}/decisions/`, decisionData);
      return response.data;
    } catch (error) {
  ...
  static async updateDecision(businesscardId, decisionId, decisionData) {
    try {
      const response = await baseService.patch(`${BASE_URL}${businesscardId}/decisions/${decisionId}/`, decisionData);
      return response.data;
    } catch (error) {
  ...
  static async deleteDecision(businesscardId, decisionId) {
    try {
      await baseService.delete(`${BASE_URL}${businesscardId}/decisions/${decisionId}/`);
    } catch (error) {
