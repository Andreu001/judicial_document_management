...
const KasSideDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Загружаем роли сторон из business_card (общий эндпоинт)
        const rolesResponse = await KasCaseService.getSideRoles();
        ...
        if (sideId && sideId !== 'create' && sideId !== 'undefined' && sideId !== 'null') {
          const data = await KasCaseService.getSideById(proceedingId, sideId);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Sending nested data:', kasSideData);

      if (sideId && sideId !== 'create' && sideId !== 'undefined' && sideId !== 'null') {
        await KasCaseService.updateSide(proceedingId, sideId, kasSideData);
      } else {
        await KasCaseService.createSide(proceedingId, kasSideData);
      }
